...
// Serves lib/resolve-locale.js verbatim to two consumers: the checkout shell
// mock (src/assets/libs/template-host.js), which runs in the host page, and
// the wrapped iframe document, which loads it as the first of its two
// <script src> tags. One shared file instead of divergent copies of the
// algorithm.
const RESOLVE_LOCALE_SCRIPT_PATH = '/lib/resolve-locale.js';
// Serves lib/template-runtime.js verbatim as the wrapped document's second
// script tag. Keeping the runtime a real, servable file (rather than a string
// a Node module builds) is what lets vcs.checkout-ui vendor a byte-for-byte
// copy and diff the two in CI.
const TEMPLATE_RUNTIME_SCRIPT_PATH = '/lib/template-runtime.js';

function invalidateWrapCache() {
  [
    // lib/template-runtime.js is deliberately absent: it is a browser script
    // this server streams from disk on every request, never require()d, so it
    // has no require.cache entry to invalidate.
    './wrap-template.js',
    './load-bundle.js',
    './preview-config.js',
    './resolve-locale.js',
...
function wrapTemplate(bundle, defaultLocale, origin, themeTokens) {
  invalidateWrapCache();
  return require('./wrap-template').wrapTemplate(bundle, defaultLocale, origin, themeTokens);
}
...
/**
 * Serves lib/template-runtime.js itself as a plain static script, the same way
 * serveResolveLocaleScript does. The path is fixed (not derived from the
 * request URL), so there is no user input to validate or contain here.
 */
function serveTemplateRuntimeScript(req, res) {
  streamFile(path.join(__dirname, 'template-runtime.js'), res);
}
...
function createPreviewMiddleware(options) {
  ...
  return function previewMiddleware(req, res, next) {
    ...
    //
    // `isIndex` is already false here, so the only `.html` this route can
    // still serve is nothing at all: the canonical index.html was handled
    // above, wrapped and with its CSP.
    //
    ...
    if (!isAllowedBundleFilename(path.basename(normalizedPath))) {
      res.statusCode = 404;
      res.end('Not Found');
      return;
    }
