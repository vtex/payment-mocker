...
const { buildThemeTokenStyle } = require('./theme-tokens');
...
/**
 * @param {Object} bundle loaded bundle (html + i18n file entries)
 * @param {string} defaultLocale fallback locale, `xx-XX`
 * @param {string} [origin] this server's own origin, for the CSP source lists
 * @param {Object} [themeTokens] optional theming tokens; see lib/theme-tokens.js
 * @returns {string}
 */
function wrapTemplate(bundle, defaultLocale, origin, themeTokens) {
  ...
  // Theming tokens ride on the root container's `style` attribute, which the
  // CSP already permits (style-src carries 'unsafe-inline'); no directive
  // changes to accommodate them, and notably no `font-src` — see the known
  // limit documented in lib/theme-tokens.js. Empty string when nothing is
  // configured, so an untokenized document stays byte-identical.
  ...
  return [
    ...
    '<div id="payment-template-i18n" data-payment-template-i18n hidden style="display:none">' +
      i18nPayload +
      '</div>',
    // The container is also where theming tokens are DECLARED, so the whole
    // fragment inherits them: a custom property set here is visible to every
    // descendant, which is what lets a partner write
    // `var(--checkout-font-family, <fallback>)` on any selector in style.css.
    // Declaring them (rather than applying `font-family` directly) is the point
    // — the bundle stays in charge of which properties actually consume the
    // value, and its fallback keeps the template whole when nothing arrives.
    '<div id="template-root" data-payment-template-root style="display:flow-root' + themeStyle + '">',
